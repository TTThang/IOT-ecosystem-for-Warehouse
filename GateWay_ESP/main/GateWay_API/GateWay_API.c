...
Packet_t Parse_Packet(char *packet){
	...
	if	   (strcmp(pkt.cmd, CMD_WIFI_SCAN) == 0)
	...
	else if(strstr(pkt.cmd, CMD_STM_LOG))
		...
		pkt.command = Command_Unknown;
//		ESP_LOGE("PACKET", "COMMAND UNKNOWN!!!!!!!!!!!!!!");
	}
...
char *GetData(char *data, uint8_t num_of_data, char brch){
	...
	index += 1;
	data0:;
	char* data_ptr = (char *)(data + index);
	if(data_ptr[0] == ' ') {
		data_ptr += 1;
		index += 1;
	}

	int olength = FindChar(data_ptr, ',');
	if(olength < 0) olength = strlen(data) - index;

	char* buffer = malloc(olength + 1);
	memcpy(buffer, data_ptr, olength);
	buffer[olength] = '\0';
